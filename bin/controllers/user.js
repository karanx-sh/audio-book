...
exports.signup = async (req, res) => {
  try {
    if (!req.body.email || !req.body.name || !req.body.number || !req.files || !req.body.password || !req.body.cpassword || req.body.password != req.body.cpassword) throw customError.dataInvalid;
    ...
    if (user) {
      ...
      if (user.status === "created" || user.status === "init") {
...
exports.verifyOTP = async (req, res) => {
  try {
    ...
    let user = await User.findOne({
      where: { number: req.body.number, status: "init" },
    });
